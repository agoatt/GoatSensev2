...
namespace HWIDSpoofer
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (key != "agoatishot")
            {
                Console.WriteLine("Invalid key. Exiting program.");
                return;
            }

            Console.WriteLine("Key accepted. Press 'n' and Enter to spoof a new HWID. Press 'k' and Enter to revert to original HWID. Press any other key to exit.");
            ...
            if (OperatingSystem.IsWindows())
            {
                using (RegistryKey registryKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64))
                {
                    using (RegistryKey subKey = registryKey.OpenSubKey(keyPath, true))
                    {
                        if (subKey != null)
                        {
                            // Exclude specific keys related to screen capture
                            if (keyPath != "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System" &&
                                keyPath != "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\SystemRestore" &&
                                !keyPath.StartsWith("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\MMDevices\\Audio\\Capture"))
                            {
